...
# Establish starting year for historical data that will not go all the way back to 2010
...
source(paste0(here::here(), "/set_environment.r"))  # particular to each person so this file is in the ignore list

if (use_live_db==TRUE){

  # load the data directly from the global TB database
  source(paste0(here::here(), "/load_data_from_database.r"))
...
source(paste0(here::here(), "/functions/round_numbers.r"), encoding = "UTF-8")
source(paste0(here::here(), "/functions/handle_NAs.r"))
source(paste0(here::here(), "/functions/get_long_data.r"))
...
# queried in a slightly different way
  ...
  rbind(get_estimates(estimates_ltbi,"e_prevtx_hh_contacts_pct", starting_year = 2018))

# remove uncertainty intervals from e_prevtx_hh_contacts_pct if value, lo and hi are all 100
adappt_est <-
  adappt_est |>
  mutate(lo = ifelse(indicator_code == "e_prevtx_hh_contacts_pct" & value == 100 & lo == 100 & hi == 100,
                        NA,
                        lo),
         hi = ifelse(indicator_code == "e_prevtx_hh_contacts_pct" & value == 100 & lo == 100 & hi == 100,
                        NA,
                        hi))


# Need to calculate aggregate values for e_prevtx_hh_contacts_pct
# Use the method from the regional and global profiles:
#
# Use the normal approximation to calculate the confidence intervals for e_hh_contacts for regional aggregates.
# The intervals for a given country is simply best +/- 1.96 * SQRT(variance)
# Therefore variance for a given country is POWER(((e_hh_contacts_hi - e_hh_contacts) / 1.96) ,2)
...
# SUM(e_hh_contacts) - 1.96 * SQRT(SUM(POWER(((e_hh_contacts_hi - e_hh_contacts) / 1.96) ,2))) AS e_hh_contacts_lo
#

# SELECT	ISNULL(SUM(newinc_con_prevtx),0) AS newinc_con_prevtx,
# 		SUM(e_hh_contacts) AS e_hh_contacts,
# 		SUM(e_hh_contacts) - 1.96 * SQRT(SUM(POWER( ((e_hh_contacts_hi - e_hh_contacts) / 1.96) ,2))) AS e_hh_contacts_lo,
# 		SUM(e_hh_contacts) + 1.96 * SQRT(SUM(POWER( ((e_hh_contacts_hi - e_hh_contacts) / 1.96) ,2))) AS e_hh_contacts_hi
#
...
adappt_temp <-
  estimates_ltbi |>
  select(iso3, year, e_hh_contacts, e_hh_contacts_lo, e_hh_contacts_hi, newinc_con_prevtx) |>
...
adappt_temp_r <-
  ...
  summarise(contacts = sum(newinc_con_prevtx, na.rm=TRUE),
...
adappt_temp <-
  adappt_temp |>
  mutate(value = cap_pct(contacts, eligible_best),
         lo = cap_pct(contacts, eligible_hi),
         hi = cap_pct(contacts, eligible_lo),
         indicator_code = "e_prevtx_hh_contacts_pct") |>
...
adappt_sdg <-
  ...
  # health expenditure per capita, reverted back to the WB for the source instead of GHO
  rbind(get_external_indicators(external_indicator_data,
                                indicator_filter = "SH.XPD.CHEX.PP.CD",
                                flg_latest_year = FALSE,
...
# make sure budget, expenditure (proxy is received funding) and population are treated as numeric, otherwise global estimates will be too big to hold ..

budget_expenditure$budget_tot <- as.numeric(budget_expenditure$budget_tot)
budget_expenditure$cf_tot <- as.numeric(budget_expenditure$cf_tot)
...
adappt_data <-
  ...
  rbind(get_vars_and_aggregates(notification, c("newrel_hivpos",
                                                "newrel_art"),
                                starting_year = hist_start_year )) |>
  rbind(get_vars_and_aggregates(budget_expenditure, "budget_tot",
                                starting_year = report_year,
                                ending_year = report_year )) |>
...
# Convert budgets and received funding to millions; show numbers < 1 million to one decimal place
adappt_data <-
  adappt_data |>
  mutate(value = ifelse(indicator_code %in% c("budget_tot", "rcvd_tot"),
                        ifelse(value > 1e6,
                               round(value / 1e6, 0),
                               round(value / 1e6, 1)),
                        value))
...
# Need to replace aggregate budgets, expenditures and percentages with cleaned versions which also
# include estimated inpatient and outpatient costs (GHS = general health service costs)
...
adappt_data <-
  ...
  filter( !(indicator_code %in% c("budget_tot", "c_f_domestic_pct", "c_f_international_pct", "c_f_unfunded_pct",
                                  "rcvd_tot", "c_rcvd_domestic_pct", "c_rcvd_international_pct") &
...
agesex_vars <- c("agegroup_option",
                 "newrel_m04", "newrel_m514", "newrel_m014",
...
adappt_agesex_notifs <-
  adappt_agesex_notifs |>
  mutate(flg_15plus_only = ifelse(agegroup_option == 223,
                                  TRUE,
                                  FALSE))
...
adappt_agesex_notifs <-
  adappt_agesex_notifs |>
  mutate(flg_014_only = ifelse(agegroup_option == 222,
                               TRUE,
                               FALSE))
