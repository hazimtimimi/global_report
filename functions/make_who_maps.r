...
# ----------------------------------------------------------
# Make a bubble chart overlayed onto the WHO map
# base code is a copy of WHOmap.print
# (One day I'll make this a bit more efficient and remove copied code ....)
# ----------------------------------------------------------
...
#   drop lines that would be whited out.
  ...
  pol3 <- geom_polygon(data = subset(gpoly, id=="Jammu and Kashmir"), aes(group = group), fill = I("grey75"), colour = NA)  # To color Jammu Kashmir
  pol4 <- geom_polygon(data = subset(gpoly, id=="Abyei"), aes(group = group), fill = I("grey75"), colour = border_colour, linetype="dotted")  # To color Abyei
  pol5 <-	geom_polygon(data = gworld[gworld$id=='ESH',], aes(group = group), fill = I("grey75"), colour = border_colour)  # To color Western Sahara
...
#   lin4 <- geom_path(data = subset(gline, id %in% c(8)), aes(group = group), colour = "white", linetype = "dotted")   # dotted white lines (8 and 9 are the same!) I'm replacing this with a new line 4...
  ...
  plot <-  ggplot(toplot, aes(long, lat)) +
            ...
            geom_polygon(aes(group=group, fill=cat), colour="white") +
            ...
            coord_cartesian(xlim = zoomx, ylim=zoomy) + labs(title = map_title) +
